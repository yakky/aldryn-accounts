...
class SignupView(FormView):
    ...
    def form_valid(self, form):
        ...
        if settings.ACCOUNTS_EMAIL_CONFIRMATION_REQUIRED:
        ...
        if not email_is_trusted and settings.ACCOUNTS_EMAIL_CONFIRMATION_REQUIRED:
            email_address_verification = EmailConfirmation.objects.request(self.created_user, email=email, send=settings.ACCOUNTS_EMAIL_CONFIRMATION_EMAIL)
        elif email_is_trusted:
            email_address = EmailAddress.objects.add_email(self.created_user, self.created_user.email)
        self.after_signup(form)
        if settings.ACCOUNTS_EMAIL_CONFIRMATION_REQUIRED and (email_is_trusted is False):
            response_kwargs = {
            ...
            show_message = [
                settings.ACCOUNTS_EMAIL_CONFIRMATION_EMAIL,
    ...
    def get_success_url(self, fallback_url=None, **kwargs):
        if fallback_url is None:
            fallback_url = settings.ACCOUNTS_SIGNUP_REDIRECT_URL
    ...
    def is_open(self):
        ...
        if code:
            try:
                ...
                if not settings.ACCOUNTS_OPEN_SIGNUP:
                    return False
                else:
            ...
            return settings.ACCOUNTS_OPEN_SIGNUP
