...
def associate_by_email_if_backend_is_trusted(details, user=None, backend=None, **kwargs):
    ...
    if user:
        return None
    if not backend.name in settings.ALDRYN_ACCOUNTS_SOCIAL_BACKENDS_WITH_TRUSTED_EMAIL:
        return None
...
def create_user(backend, details, response, uid, username, user=None, request=None, **kwargs):
    ...
    if email and backend.name in settings.ALDRYN_ACCOUNTS_SOCIAL_BACKENDS_WITH_TRUSTED_EMAIL:
